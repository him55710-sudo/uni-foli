...
from polio_api.services.pdf_analysis_service import build_student_record_structure_metadata
...
def test_build_student_record_structure_metadata_extracts_core_fields() -> None:
    parsed = ParsedDocumentPayload(
        ...
        word_count=300,
        content_text=(
            "2학년 1학기 교과학습발달상황에서 데이터 분석 탐구를 수행함. "
            "후속 활동으로 비교 실험을 진행했고 과정과 한계를 성찰함. "
            "진로 연계 문장을 통해 전공 적합성을 설명함."
        ),
        content_markdown="",
        metadata={},
        ...
        raw_artifact={
            "pages": [
                {"page_number": 1, "text": "교과학습발달상황 세부능력 특기사항"},
                {"page_number": 2, "text": "창의적 체험활동 동아리 진로활동"},
                {"page_number": 3, "text": "행동특성 및 종합의견"},
            ]
        },
        masked_artifact={},
        analysis_artifact={},
